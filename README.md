...
[![huggingface](https://img.shields.io/badge/🤗-LLAMA--3.2-yellow)](https://huggingface.co/spaces/AhmedSSabir/demo-for-Visual-Re-ranker-LLLMA-3.2-1B)
...
-->
1. Hypothesis (caption candidates beam search) $\text{P}(w)$  initialized by common observation (_i.e._ language model)
<!--
...
3. Similarities $\alpha=\left[\frac{1 - \text{sim}(w, c)}{1+\text{sim}(w, c)}\right]^{1-\text{P}(c)}$ the relatedness between the two concepts (visual context and hypothesis) with respect to the informativeness of the visual information.
